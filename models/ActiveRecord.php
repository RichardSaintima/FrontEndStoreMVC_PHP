...
class ActiveRecord {
    ...
    public static function whereInt($columna, $valor) {
        $query = "SELECT * FROM " . static::$tabla . " WHERE ${columna} = '${valor}'";
        $resultado = self::consultarSQL($query);
        return ( $resultado ) ;
    }
    ...
    public static function totalSum($columna = '',$tablabusqueds = '', $valor1 = '', $valor2 = '') {
        $query = "SELECT SUM($columna) FROM " . static::$tabla . " JOIN ${tablabusqueds} ON ${valor1} = ${valor2}";
        $resultado = self::$db->query($query);
        $total = $resultado->fetch_array();
    
        return (float) array_shift($total);
    }
